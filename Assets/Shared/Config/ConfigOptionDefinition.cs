...
namespace Garrison.Shared.Config
{
    ...
    {
        public ConfigOptionDefinition(ConfigKey key, ConfigValueType type, string group, string label, float min = 0f, float max = 0f, bool hasRange = false)
        {
            Key = key;
            Type = type;
            Group = group;
    ...
    {
        public static IReadOnlyList<ConfigOptionDefinition> All { get; } = new[]
        {
            ...
            Float(ConfigKey.CameraSafeViewportInset, "Camera", "Safe viewport inset", 0f, 0.45f),
        ...
        private static ConfigOptionDefinition Int(ConfigKey key, string group, string label, int min, int max = int.MaxValue)
        {
            return new ConfigOptionDefinition(key, ConfigValueType.Int, group, label, min, max, true);
        }

        private static ConfigOptionDefinition Float(ConfigKey key, string group, string label, float min, float max = float.MaxValue)
        {
            return new ConfigOptionDefinition(key, ConfigValueType.Float, group, label, min, max, true);
        }
